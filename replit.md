...
- **Shoot Brief System**: Dedicated page at `/shoot-briefs` for logging visual direction and notes for shoots. Shows today's shoots as cards with green/grey brief status indicators. Admin can upload reference and inspiration images with captions and add editing notes. Retouchers and Data Wranglers view briefs read-only. Each project in the task table displays a camera icon (green = brief exists, grey = none) linking directly to that project's brief. DB tables: `shoot_briefs` (1:1 with projects) and `shoot_brief_images` (multiple per brief, typed as 'reference' or 'inspiration').
