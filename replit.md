...
- **AI-Powered Features** (via OpenAI/Replit AI Integrations, `server/services/aiService.ts`):
  ...
  - **Quality Gate**: Mandatory AI photo review before delivery. Fully automated pipeline: when Drive Monitor detects upload complete (photo count matches selectedCount), it automatically triggers AI quality check (gpt-4o vision, threshold score 7/10). If quality PASSES: preview email sent with gallery link (no access yet) → folder made public → delivery email sent with access → project marked Delivered. If quality FAILS: AI sends personalized feedback message to the assigned retoucher via AI chat explaining issues. When retoucher re-uploads (photo count changes), quality gate auto-resets for re-check. Admin can override failed quality gate. Reference images from Instagram/portfolio can be uploaded via Drive Manager UI and are used as benchmarks by the AI. Schema fields: `qualityGateScore`, `qualityGatePassed`, `qualityGateOverride`, `drivePreviewEmailSent` on projects table. APIs at `/api/ai/quality-gate`, `/api/ai/quality-gate/override`, `/api/quality-reference-images`. Configurable via `quality_gate_settings` and `quality_reference_images` app settings.
