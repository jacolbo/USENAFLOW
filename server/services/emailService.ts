...
// Email 5: Removed - Assignment Welcome merged with Chat Link email (Email 3)
...
export async function sendRescheduleEmail(
  ...
  const oldFormatted = formatSAST(new Date(oldDate));
  const newFormatted = formatSAST(new Date(newDate));

  const subject = `Update on your photo delivery, ${firstName}`;
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #fff;">
      <div style="text-align:center;padding:16px 0 24px">
        <strong style="font-size:20px;letter-spacing:2px;color:#2c2c2c">JEPSON MYLES STUDIO</strong>
      </div>
      <p style="color:#2c2c2c;font-size:16px;line-height:1.6;margin:0 0 16px">Hi ${firstName},</p>
      <p style="color:#2c2c2c;font-size:16px;line-height:1.6;margin:0 0 16px">
        We wanted to let you know that your photo delivery date has been updated.
      </p>
      <table style="width:100%;border-collapse:collapse;margin:20px 0;background:#f9f9f9;border-radius:8px;overflow:hidden">
        <tr>
          <td style="padding:12px 16px;font-size:14px;color:#888;border-bottom:1px solid #eee">Original delivery date</td>
          <td style="padding:12px 16px;font-size:14px;color:#2c2c2c;border-bottom:1px solid #eee"><s>${oldFormatted}</s></td>
        </tr>
        <tr>
          <td style="padding:12px 16px;font-size:14px;color:#888">New delivery date</td>
          <td style="padding:12px 16px;font-size:15px;font-weight:bold;color:#2c2c2c">${newFormatted}</td>
        </tr>
      </table>
      <p style="color:#2c2c2c;font-size:16px;line-height:1.6;margin:0 0 16px">
        We apologise for any inconvenience and appreciate your patience. 
        Our team is working hard to deliver your photos beautifully.
      </p>
      <p style="color:#2c2c2c;font-size:15px;line-height:1.6;margin:28px 0 4px">Warm regards,</p>
      <p style="color:#2c2c2c;font-size:15px;line-height:1.6;margin:0"><strong>Jepson Myles Studio</strong></p>
    </div>
  ...
  try {
    ...
    const response = await client.emails.send({ from: fromEmail, replyTo, to: clientEmail, subject, html: htmlContent });
    if (response.data?.id) {
      await logEmail(projectId, 'reschedule' as any, clientEmail, subject, 'sent', response.data.id);
      return { success: true, messageId: response.data.id };
    }
    const errorMsg = response.error?.message || 'Unknown error';
    await logEmail(projectId, 'reschedule' as any, clientEmail, subject, 'failed', undefined, errorMsg);
    return { success: false, error: errorMsg };
  } catch (error: any) {
    await logEmail(projectId, 'reschedule' as any, clientEmail, subject, 'failed', undefined, error.message);
    return { success: false, error: error.message };
  }
}
