...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/drive/release/:projectId", verifyAdminRequest, async (req, res) => {
    try {
      const { projectId } = req.params;
      const project = await storage.getProject(projectId);
      if (!project) return res.status(404).json({ error: "Project not found" });
      ...
      const updateData: any = {
        driveGalleryLink: shareLink,
        galleryLink: shareLink,
        driveAccessGranted: true,
        driveAccessGrantedAt: new Date(),
        driveDeliveryComplete: true,
        driveDeliveryCompletedAt: new Date(),
        status: ProjectStatus.DELIVERED,
        deliveredAt: new Date(),
        deliveryApproved: true,
        deliveryApprovedAt: new Date(),
        deliveryApprovedBy: req.body.approvedBy || 'Admin',
      };
      ...
      if (project.clientEmail) {
        try {
          ...
          console.error('[Email] Error sending delivery email:', emailError);
        }
        ...
        try {
          ...
          console.error('[Survey] Error creating/sending survey:', surveyError);
        }
      }
      ...
      try {
        await storage.recordStatusTransition(projectId, 'Review', ProjectStatus.DELIVERED, updateData.deliveryApprovedBy);
      } catch (e) {}

      res.json({ success: true, galleryLink: shareLink });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
