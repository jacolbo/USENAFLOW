...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const { verifyShootTrackerAccess } = await import("./middleware/adminAuth");
  
  app.get("/api/auth/google", verifyShootTrackerAccess, async (req, res) => {
    try {
      const { getAuthorizationUrl } = await import("./services/googleCalendar");
  ...
  app.get("/api/auth/google/callback", async (req, res) => {
    try {
      ...
      const { validateOAuthState, handleAuthCallback } = await import("./services/googleCalendar");
        ...
        return res.status(400).send(`
          <html>
            ...
            <body style="font-family: sans-serif; text-align: center; padding: 50px;">
              ...
              <p>The authorization request is invalid or expired. Please try again.</p>
            </body>
          </html>
        `);
      }
      ...
      res.send(`
        <html>
          ...
          <body style="font-family: sans-serif; text-align: center; padding: 50px;">
            ...
            <p>You can now close this window and return to USENA FLOW.</p>
  ...
  app.get("/api/auth/google/status", verifyShootTrackerAccess, async (req, res) => {
    try {
      const { isGoogleConnected, listCalendars } = await import("./services/googleCalendar");
      ...
      res.json({ connected, calendars });
    } catch (error: any) {
      console.error("Google status check error:", error);
      res.json({ connected: false, calendars: [], error: error.message });
    }
  });
